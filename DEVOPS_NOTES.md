...
## 🔴 Incident: git restore اشتباه باعث crash کامل production شد (2026-09-06)

**بازه‌ی زمانی:** حدود ۰۸:۴۵ تا ۰۹:۱۰ (UTC) — کل سرور production
با `FUNCTION_INVOCATION_FAILED` روی **هر** درخواست (حتی `GET /health`
که هیچ منطقی ندارد) از کار افتاد.

**علت واقعی:** حین پاک‌سازی یک branch آزمایشی (Gemini provider
evaluation)، یک `git restore src/app.js` برای کنار زدن یک تغییر
noise/ناخواسته زده شد. این دستور به‌جای برگرداندن فایل به آخرین
commit درست، آن را به یک حالت میانی/قدیمی‌تر برگرداند که هنوز
`require('./routes/testGeminiRoutes')` را داشت — فایلی که در یک
commit قبلی‌تر حذف شده بود. چون `git diff` بعد از restore فقط دو
خط (همان‌هایی که قبلاً حذف کرده بودیم) را به‌عنوان "modified" نشان
داد، به‌نظر می‌رسید این یک بازگشت بی‌ضرر است — ولی در واقع محتوای
واقعی فایل بررسی نشد، فقط ظاهر diff.

این نسخه‌ی خراب، بدون تشخیص، در merge نهایی به `main` وارد شد.
`require()` یک ماژول ناموجود در سطح بارگذاری فایل (module-level،
نه داخل یک تابع) اجرا می‌شود — یعنی Node قبل از رسیدن به Express
یا هر route، بلافاصله throw می‌کند و کل فرآیند crash می‌کند.

**قاعده‌ای که باید همیشه رعایت شود:** بعد از هر `git restore`
(یا هر عملیات دیگری که محتوای یک فایل را از یک نسخه‌ی دیگر
بازمی‌گرداند — `git checkout <commit> -- <file>`، `git stash pop`
و غیره)، **همیشه محتوای کامل فایل را دوباره بخوان و verify کن**،
نه فقط به خلاصه‌ی `git diff`/`git status` اعتماد کن. این دومین بار
در همین روز است که یک عملیات git (اولی: `prisma migrate dev` که
نزدیک بود کل schema را reset کند؛ این یکی: `git restore`) باعث یک
مشکل جدی شده — الگو نشان می‌دهد عملیات‌های "بازگردانی" (restore/
reset/checkout به نسخه‌ی دیگر) نیاز به یک قدم تأیید صریح دارند،
نه فرض بر اساس این‌که "این باید کار درستی باشد".
